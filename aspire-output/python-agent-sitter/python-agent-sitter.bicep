...
param python_agent_sitter_containerimage string

param foundryprojecturl_value string

param octopets_appinsights_outputs_appinsightsconnectionstring string

resource python_agent_sitter 'Microsoft.App/containerApps@2025-01-01' = {
  name: 'python-agent-sitter'
  location: location
  properties: {
    configuration: {
    ...
    template: {
      containers: [
        {
          image: python_agent_sitter_containerimage
          name: 'python-agent-sitter'
          env: [
            {
