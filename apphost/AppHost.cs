...
var agent = builder.AddPythonScript("chat", "../agent", "agent.py")
    .WithUvEnvironment()
    .WithHttpsEndpoint(targetPort: 8001)
...
var sitter_agent = builder.AddPythonScript("sitter", "../sitter-agent", "app.py")
    .WithUvEnvironment()
    .WithHttpsEndpoint(targetPort: 8002)
    .WithEnvironment("AZURE_OPENAI_ENDPOINT", foundryProject)
    .WithAzureUserAssignedIdentity(identity)
    .WithIconName("ChatEmpty")
    .PublishAsDockerFile()
...
var orchestrator = builder.AddPythonScript("orchestrator", "../orchestrator-agent", "app.py")
    .WithUvEnvironment()
    .WithHttpsEndpoint(targetPort: 8003)
    .WithEnvironment("AZURE_OPENAI_ENDPOINT", foundryProject)
    .WithEnvironment("LISTINGS_AGENT_URL", agent.GetEndpoint("https"))
    .WithAzureUserAssignedIdentity(identity)
    .WithEnvironment("SITTER_AGENT_URL", sitter_agent.GetEndpoint("https"))
...
var frontend = builder.AddNpmApp("frontend", "../frontend")
    ...
    .WithEnvironment("REACT_APP_AGENT_API_URL", agent.GetEndpoint("https"))
    .WithEnvironment("REACT_APP_SITTER_AGENT_API_URL", sitter_agent.GetEndpoint("https"))
    .WithEnvironment("REACT_APP_ORCHESTRATOR_API_URL", orchestrator.GetEndpoint("https"))
